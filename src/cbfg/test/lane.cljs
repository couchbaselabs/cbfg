...
(defn test-lane-pump [actx]
  (act tlp actx
       (let [n (atom 0)]
         (if (and
              (let [in-ch (achan actx)
                    out-ch (achan actx)
                    g (make-lane-pump tlp in-ch out-ch make-echo-lane)]
                (and (do (aput tlp in-ch {:lane :a :op :hi :x 1})
                         (e n
                            (atake tlp out-ch)
              ...
              (let [in-ch (achan actx)
                    ...
                    g (make-lane-pump tlp in-ch out-ch make-echo-lane
                                      :max-lanes 0)]
                (do (aput tlp in-ch {:lane :a :op :hi :x 1})
                    (e n
                       (atake tlp out-ch)
                       {:lane :a :op :hi :x 1
                        :status :full :status-info :max-lanes}
                       nil)))
              (let [in-ch (achan actx)
                    out-ch (achan actx)
                    g (make-lane-pump tlp in-ch out-ch make-echo-lane
                ...
                (and
                 (do (aput tlp in-ch {:lane :a :op :hi :x 1})
                         (e n
                            ...
                            [:a 0 {:lane :a :op :hi :x 1}]
                            nil))
                 (do (aput tlp in-ch {:lane :b :op :hi :x 2})
                     (e n
                        (atake tlp out-ch)
                        {:lane :b :op :hi :x 2
                         :status :full :status-info :max-lanes}
